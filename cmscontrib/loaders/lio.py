...
class LioTaskLoader(TaskLoader):
    ...
    def parse_points_and_public_groups(self, points):
            ...
            if isinstance(public, bool):
                ...
                for group in public:
                    if group not in groups:
                        raise ValueError(
                            f"Provided group number {group} not in this test group block {groups}"
                        )
                    public_groups.add(group)

            attachment = row.get("attachment")
            if attachment is not None:
                input = attachment.get("input", False)
                output = attachment.get("output", False)
                if not input:
                    output = False
                for group in groups:
                    attachment_groups[group] = {"input": input, "output": output}
        ...
        return points_per_group, public_groups, attachment_groups
    ...
    def get_task(self, get_statement):
        ...
        if get_statement:
            ...
            if task_args["statements"]:
                task_args["primary_statements"] = self.conf.get(
                    "primary_statements", ["lv"]
                )
        ...
        set_if_present(
            ...
            task_args,
            "submission_format",
            conv=submission_format_convert,
            default=submission_format_convert(name),
        )
        ...
        if "tests_groups" in self.conf:
            points_per_group, public_groups, attachment_groups = (
                self.parse_points_and_public_groups(self.conf["tests_groups"])
            )
        else:
            point_file = os.path.join(
                self.task_dir, self.conf.get("point_file", "punkti.txt")
            )
        ...
        args["score_type_parameters"] = [
            [points_per_group[i], f"{i:0{max_group_digit_length}}"]
            for i in range(len(points_per_group))
        ]

        if is_communication:
            args["task_type_parameters"] = [1, "alone", "std_io"]
        else:
        ...
        test_zip = os.path.join(
            self.task_dir, self.conf.get("tests_archive", "testi.zip")
        )
            ...
            for group in sorted(test_files.keys()):
                for test_in_group in sorted(test_files[group].keys()):
                    ...
                    if "input" not in testcase or "output" not in testcase:
                        raise LioLoaderException(
                            f"Input or output file not found for test {group}{test_in_group}"
                        )
                    with zip.open(testcase["input"], "r") as input_file:
                        content = io.TextIOWrapper(
                            input_file, encoding="ascii", newline=None
                        ).read()
                    ...
                    with zip.open(testcase["output"], "r") as output_file:
                        content = io.TextIOWrapper(
                            output_file, encoding="ascii", newline=None
                        ).read()
...
class LioContestLoader(ContestLoader):
    ...
    def get_contest(self):
        ...
        args["start"] = to_datetime(
            self.conf.get("start", datetime.datetime(1970, 1, 1))
        )
