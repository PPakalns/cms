...
NORETURN void InStream::quit(TResult result, const char* msg)
{
    ...
    if (mode != _output && result != _fail && !(testlibMode == _interactor && mode == _input))
    {
...
void registerInteraction(int argc, char* argv[])
{
    ...
    static std::filebuf out;
    std::cout.rdbuf(out.open(argv[2], std::ios_base::out));

    ouf.opened = false;
    ouf.name = "output";
    ouf.mode = _output;
    ouf.stdfile = true;
    ouf.reset(fopen(argv[1], "rb"));

    tout.open("output.txt", std::ios_base::out);
